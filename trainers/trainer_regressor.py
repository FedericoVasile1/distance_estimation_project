...
from utils.plots import (average_variance_plot, bias_plot,
                         distance_bins_plot, visibility_bins_plot)
from utils.sampler import CustomSamplerTest, CustomSamplerTrain, CustomDistributedSampler
...
class TrainerRegressor(Trainer):
    ...
    def train(self) -> None:
        ...
        scaler = torch.cuda.amp.GradScaler(init_scale=(
            2 ** 16) / self.cnf.accumulation_steps, enabled=self.cnf.fp16)
        ...
        with tqdm(total=len(self.train_loader), desc=f'Epoch {self.epoch + 1}/{self.cnf.epochs}') as pbar:
            for step, sample in enumerate(self.train_loader):
                ...
                y_true = torch.cat([d[-1]
                                   for d in distances]).to(self.cnf.device)
                ...
                y_pred_z = nearness_to_z(torch.tensor(
                    output)).numpy() if self.cnf.nearness else output
                ...
                pbar.set_postfix({'alp@1': epoch_acc,
                                  'loss': np.mean(losses['loss']),
                                  'lr': self.optimizer.param_groups[0]['lr']})
                pbar.update()
                if wandb.run:
                    wandb.log({'it_train_acc': hits / len(y_true)} |
        ...
        if wandb.run:
            wandb.log({'epoch': self.epoch,
                       ...
                       'train_it': (step + 1) * (self.epoch + 1)} | {f'epoch_train_{k}': np.mean(v) for k, v in
                                                                     losses.items()})
    ...
    def test(self) -> None:
        ...
        with tqdm(total=len(self.test_loader), desc='Testing') as pbar:
            for step, sample in enumerate(self.test_loader):
                ...
                x, _, _, video_bboxes, distances, visibilities, classes, _, frame_idx, video_idx, _ = sample
                ...
                all_visibilities = np.append(
                    all_visibilities, last_frame_visibilities.tolist())
                all_classes = np.append(
                    all_classes, last_frame_classes.tolist())
                dists_pred = nearness_to_z(np.asarray(dists_pred)).tolist(
                ) if self.cnf.nearness else dists_pred

                all_pred = np.append(all_pred, dists_pred)
                errors = np.abs(
                    np.array(last_frame_distances) - np.array(dists_pred))
                ...
                pbar.update()
                pbar.set_postfix({'alp@1': hits / tot,
                                  'loss': tot_loss / (step + 1)})

        all_results = {'all_errors': all_errors,
                       'all_true': all_true,
                       'all_vars': all_vars,
                       'all_pred': all_pred,
        ...
        metrics = get_metrics(y_true=all_true, y_pred=all_pred, y_visibilities=all_visibilities, y_classes=all_classes,
        ...
        if wandb.run:
            ...
            distance_bins_plot(all_errors=all_errors,
                               all_true=all_true, acc=acc)
        ...
        if wandb.run:
            wandb.log({'patience': self.patience, 'epoch': self.epoch,
                      'best_rmse': self.best_test_rmse_linear})

        if self.cnf.rank == 0:
            print(f'\r \t● (ACC) on TEST-set: '
                  f'({acc:.2%}) '
    ...
    def get_dataset(self, args: argparse.Namespace) -> Tuple[Dataset, Dataset]:
        ...
        if not self.cnf.test_only:
            ...
            self.train_loader = DataLoader(
                dataset=training_set, batch_size=args.batch_size,
                num_workers=args.n_workers, pin_memory=True,
                worker_init_fn=training_set.wif, drop_last=True,
                collate_fn=training_set.collate_fn, sampler=train_sampler)

        if not self.cnf.distributed:
            test_sampler = CustomSamplerTest(
                test_set, stride=args.test_sampling_stride)
        else:
            test_sampler = CustomDistributedSampler(test_set, train=False,
                                                    stride=args.test_sampling_stride)
        self.test_loader = DataLoader(
            dataset=test_set, batch_size=1, pin_memory=True,
            worker_init_fn=test_set.wif_test, num_workers=args.n_workers,
