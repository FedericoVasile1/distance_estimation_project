...
MODELS = {'zhu': partial(ZHU, enhanced=False),
...
def parse_args(default: bool = False) -> argparse.Namespace:
    ...
    parser.add_argument('--annotations_path', type=str, default='npy_annotations/annotations_clean')
    ...
    parser.add_argument("--pretrain", default="imagenet",
                        type=str, choices=("none", "imagenet"))
    parser.add_argument('--use_centers', action='store_true', default=True)
    parser.add_argument('--no_use_centers',
                        dest='use_centers', action='store_false')
    parser.add_argument('--residual_centers',
    ...
    parser.add_argument('--alpha_zhu', type=float,
                        default=1e-2, help='Zhu enhanced loss weight')

    # Non-Local Block
    parser.add_argument('--phi_ksize', type=int, default=1,
                        choices=(1, 3), help='[NLB] Kernel size for phi')
    parser.add_argument('--batch_norm_nlb', action='store_true',
                        default=True, help='[NLB] Use batch norm in NLB')
    parser.add_argument('--no_batch_norm_nlb',
                        dest='batch_norm_nlb', action='store_false')
    parser.add_argument('--sub_sample_nlb', action='store_true',
                        default=False, help='[NLB] Use sub-sampling in NLB')
    parser.add_argument('--nlb_mode', type=str, default='gaussian', choices=('gaussian', 'dot', 'embedded'),
                        help='[NLB] Mode for NLB')

    # Inference
    parser.add_argument('--test_only', action='store_true',
                        default=False, help='Test only without training')
    ...
    parser.add_argument('--input_h_w', type=int,
                        nargs='+', default=(720, 1280))
    parser.add_argument('--min_bbox_hw', type=int, nargs='+', default=(8, 8))
    parser.add_argument('--min_visibility', type=float,
                        default=0.18)  # 0.18 \approx 4/22
    parser.add_argument('--crop_range', type=float,
                        nargs='+', default=(0.0, 0.25))
    ...
    parser.add_argument('--sampling', type=str,
                        default='naive', choices=('naive', 'smart'))
    ...
    parser.add_argument('--aug_gaussian_blur_sigma',
                        nargs='+', type=float, default=(2.0, 4.0))
...
def set_default_args(args) -> argparse.Namespace:
    ...
    if not temporal and args.clip_len > 1:
        print(YELLOW + "The selected model does not support temporal data, falling back to clip_len=1" + END + '\n')
...
def set_exp_name(args: argparse.Namespace):
    if args.exp_name is None:
        ...
        if args.resume:
            assert args.resume and args.exp_name is not None, 'Cannot resume without --exp_name'


def preprocess_args(args):
    assert Path(args.ds_path).exists(
    ), f'Dataset path {args.ds_path} does not exist'
...
def main(args: argparse.Namespace) -> None:
    ...
    assert not (args.resume and args.checkpoint), 'Cannot use both resume and checkpoint'
